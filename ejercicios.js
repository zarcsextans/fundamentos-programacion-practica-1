...
function encontrarMaximo(numeros) {
  let maximo = numeros[0]; // asumimos que el primero es el mayor al inicio
...
const transpuesta = [];
  ...
  for (let i = 0; i < matriz[0].length; i++) {  // columnas → filas
    const fila = [];
    for (let j = 0; j < matriz.length; j++) {   // filas → columnas
